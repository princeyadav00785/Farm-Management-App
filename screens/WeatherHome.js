    ...
    const fetchUserProfile = async () => {
      try {
        const response = await axios.get(
          `http://192.168.246.200:8000/profile/${userId}`
        );
      ...
      />
       <View
        ...
        <View style={{paddingHorizontal: 20, marginTop: 100}}>
          <Text style={{fontSize: 40, color: 'white'}}>Hello {user?.name}</Text>
          ...
          <Text style={{color: 'white', fontSize: 25, paddingHorizontal: 10, marginTop: 150, marginBottom: 20}}>
            My Locations
          </Text>
          <FlatList
          horizontal
